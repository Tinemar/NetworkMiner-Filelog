...
namespace NetworkMiner {
    ...
    public partial class NetworkMinerForm : Form {
        public static class MongoAddress
        {
            private static string currentDirectory = Directory.GetCurrentDirectory();
            private static string ip = System.IO.File.ReadAllText(currentDirectory + "/config.txt");
            public static string MongodbAddress = "mongodb://" + ip + ":27017";
        };
        ...
        public NetworkMinerForm(bool showWinPcapAdapterMissingError, bool removeBrowsersAndVoipTab) {
            ...
            if (PacketParser.Utils.Logger.CurrentLogLevel == PacketParser.Utils.Logger.LogLevel.Debug && PacketParser.Utils.Logger.LogToFile) {
                var isoFileStore = PacketParser.Utils.Logger.GetIsolatedStorageFile();
            ...
            InitializeComponent();
            PacketParser.Utils.Logger.Log("Component Initialized", System.Diagnostics.EventLogEntryType.Information);
            ...
            PacketParser.Utils.Logger.Log("Requiring File IO Permissions", System.Diagnostics.EventLogEntryType.Information);
            try {
            ...
            PacketParser.Utils.Logger.Log("Checking for Mono", System.Diagnostics.EventLogEntryType.Information);
            ...
            PacketParser.Utils.Logger.Log("Checking for possible DLL injections", System.Diagnostics.EventLogEntryType.Information);
            ...
            if (NetworkWrapper.Utils.Security.DllHijackingAttempted(dllDirectories, winPcapDllFiles, out hijackedPath)) {
                ...
                PacketParser.Utils.Logger.Log("Loading WinPCAP Adapters", System.Diagnostics.EventLogEntryType.Information);
                try {
            ...
            PacketParser.Utils.Logger.Log("Loading Raw Sockets Adapters", System.Diagnostics.EventLogEntryType.Information);
            ...
            */
            ...
            PacketParser.Utils.Logger.Log("Adding Encoders", System.Diagnostics.EventLogEntryType.Information);
            ...
            PacketParser.Utils.Logger.Log("Loading cleartext dictionary", System.Diagnostics.EventLogEntryType.Information);
        ...
        private void GuiUpdateTimer_Tick(object sender, EventArgs e) {
            if (this.IsHandleCreated) {
                ...
                if (this.FileQueue.Count > 0) {
                    ...
                    //StreamWriter sw = new StreamWriter(str3+@"filelog.txt", true);
                    ...
                    //fList传给AddFilesToFileList，界面还要加上里面的其他参数
        ...
        private void AddFilesToFileList(IEnumerable<PacketParser.FileTransfer.ReconstructedFile> files) {
            ...
            //StreamWriter sw = new StreamWriter(str3 + @"filelog.txt", true);
            //直接写入数据库filelog-filelog
            var client = new MongoClient(MongoAddress.MongodbAddress.ToString());
            var database = client.GetDatabase("filelog");
            var collection = database.GetCollection<BsonDocument>("filelog");
...
2. Start NetworkMiner with administrator rights.Changedddddddddddddddddddd!");
            ...
            if (keyword.Length < 3) {
                ...
                if (this.keywordTextBox.Text.StartsWith("0x")) {//hex string
                    try {
                        ...
                        //L鋑g till keywordet till PacketHandler.PacketHandler!!!
                    }
                    catch (Exception ex) {
...
#region ListViewSorting
        ...
        private void hostSortOrderComboBox_SelectedIndexChanged(object sender, EventArgs e) {
            //H腞 SKA detailsHeader LIGGA Enabled MASSA OLIKA SORTERINGSORDNINGAR:
