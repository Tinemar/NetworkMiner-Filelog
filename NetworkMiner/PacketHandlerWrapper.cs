    ...
    public class PacketHandlerWrapper {
        ...
        internal PacketHandlerWrapper(NetworkMinerForm parentForm, System.IO.DirectoryInfo outputDirectory, List<PacketParser.Fingerprints.IOsFingerprinter> preloadedFingerprints) {
            ...
            if(this.parentForm == null || this.parentForm.GuiProperties == null)
                this.packetHandler = new PacketParser.PacketHandler(exePath, outputDirectory.FullName, preloadedFingerprints, false, NetworkMiner.GuiProperties.ToDefaultTimeZoneString);
            else
                this.packetHandler = new PacketParser.PacketHandler(exePath, outputDirectory.FullName, preloadedFingerprints, false, this.parentForm.GuiProperties.ToCustomTimeZoneString);
        ...
        internal void SnifferPacketReceived(object sender, NetworkWrapper.PacketReceivedEventArgs packet) {
            if(packetHandler.TryEnqueueReceivedPacket(sender, packet)) {
                ...
                if(this.pcapWriter!=null)
                    this.pcapWriter.WriteFrame(new NetresecShared.Pcap.PcapFrame(packet.Timestamp, packet.Data, pcapWriter.DataLinkType));
            }
